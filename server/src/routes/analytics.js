...
router.get('/', async (req, res) => {
  try {
    ...
    const avgBuildTime = total
      ? deployments.filter(d => d.buildTimeMs).reduce((a, d) => a + d.buildTimeMs, 0) / deployments.filter(d => d.buildTimeMs).length || 0
      : 0
    const byFramework = {}
    const projects = await Project.find({ userId: req.user._id })
    for (const p of projects) {
      const count = deployments.filter(d => d.projectId.equals(p._id)).length
      byFramework[p.framework || 'unknown'] = (byFramework[p.framework || 'unknown'] || 0) + count
    }
    res.json({
